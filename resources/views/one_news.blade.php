...
@section('content')
@foreach ($info as $i)
<img src="{{asset("images/".$i->image)}}" class="one_new_img">
<div class="one_new_back">
    <button class="c2 r1" onclick="location.href=`{{route('all_news')}}`">Назад</button>
</div>
<div class="one_new_date">
    <div class="c2 r1" ><?=substr($i->created_at, 8, 2).".".substr($i->created_at, 5, 2).".".substr($i->created_at, 0, 4)?></div>
</div>
<div class="one_new_name">
    <div class="c2 r1" >{{$i->title}}</div>
</div>
<div class="void"></div>
